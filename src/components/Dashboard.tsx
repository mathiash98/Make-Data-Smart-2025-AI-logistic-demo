...
export default function Dashboard() {
	...
	const { faqs, fetchAllFAQs, insertFAQ, updateFAQ, deleteFAQ } = useFAQ();
	const { partners, fetchAllPartners, insertPartner, deletePartner } =
		usePartners();
	const { properties, fetchAllProperties, insertProperty, deleteProperty } =
		useProperties();
	...
	const handleFollowUpUnsolvedTasks = async () => {
		setFollowUpLoading(true);
		try {
			await fetch(import.meta.env.VITE_WEBHOOK_URL_FOLLOW_UP_TASKS, {
				method: "POST",
		...
		<div className="h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100 flex flex-col overflow-auto">
			...
			<div className="flex-shrink-0 p-6 pb-4">
				<h1 className="text-3xl font-bold text-slate-800 mb-4">
					Proptonomy demonstration dashboard
				</h1>
				<div className="flex flex-wrap gap-4 items-center">
					...
					<Button
						onClick={handleFollowUpUnsolvedTasks}
