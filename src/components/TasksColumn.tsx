...
import type { Enums, Tables } from "@/hooks/supabase";
	...
	return (
		<ListColumn title="Tasks" count={tasks?.length || 0}>
			<div className="flex flex-col h-full min-h-0">
				<div
					className={`space-y-2 overflow-auto ${selectedTask ? "max-h-[50%] flex-shrink-0" : "flex-1"} pr-1`}
				>
					{tasks?.map((task) => (
						<div
							key={task.id}
				...
				{selectedTask && (
					<div className="border-t border-slate-200 pt-4 flex-1 min-h-0 overflow-auto pr-1">
